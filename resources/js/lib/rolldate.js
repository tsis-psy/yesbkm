...
(function (global, factory) {
  ...
  Rolldate.prototype = {
    ...
    baseData: function baseData() {
      return {
        ...
        opts: { //플러그인 기본설정
          ...
          minStep: 1,
          trigger: 'tap',
          lang: { title: '이름', cancel: '취소', confirm: '확인', year: '년', month: '월', day: '일', hour: '시', min: '분', sec: '초' }
        }
      };
    },
    ...
    createUI: function createUI() {
      ...
      var $html = '<div class="rolldate-mask"></div>\n<div class="rolldate-panel">\n<div class="pop-header"><span>'+ lang.title +'</span><button type="button" name="button" class="rolldate-cancel" data-focus="rolldate-cancel">닫기</button></div>\n<section class="rolldate-content">\n<div class="rolldate-dim mask-top"></div>\n<div class="rolldate-dim mask-bottom"></div>\n<div class="rolldate-wrapper">\n' + ul + '\n</div>\n</section>\n<div class="btn-wrap fixed-btn-wrap type-popup"><a href="#none" class="btn rolldate-confirm">확인</a></div>\n</div>',
      ...
      var _loop = function _loop(_i) {
        ...
        that.on('scrollEnd', function () {
          if (config.moveEnd) {
            ...
            // active 추가
            const liList = document.querySelectorAll('.wheel-item');
            for (let i = 0; i < liList.length; i++) {
                let transformValue = liList[i].style.transform
                if (transformValue === 'rotateX(0deg)') {
                    liList[i].classList.add('active')
                } else {
                  liList[i].classList.remove('active'); // 2023-07-12 else구문 추가
                }
            }
          }
    ...
    event: function event() {
      ...
      _this.tap(confirm, function () {
        ...
        if (config.el) {
          el = $(config.el).querySelector('.option');
          if (el.nodeName.toLowerCase() == 'input') {
            el.value = date;
          } else {
            el.innerText = date;
