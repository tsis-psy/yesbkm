...
var PDFPageView = (function PDFPageViewClosure() {
  ...
  PDFPageView.prototype = {
    ...
    draw: function PDFPageView_draw() {
      ...
      // canvas.setAttribute("alt", "?��? ?�명?? ?��?�? ?�니??." + this.id);

      //!< 2021-11-22 ?�진
      if(this.id == 1)
        canvas.setAttribute("alt", "개인(?�용)?�보 ?�집/?�용 ?�공 ?�약 ?�의??");

        if(this.id == 2)
        canvas.setAttribute("alt", "?�공 ??��");

        if(this.id == 3)
        canvas.setAttribute("alt", "?�집?�용/?�공 ?�항");

        if(this.id == 4)
        canvas.setAttribute("alt", "조회?? �??? ?�항");

        if(this.id == 5)
        canvas.setAttribute("alt", "개인(?�용)?�보 ??3?? ?�공 ?�약 ?�의??");

        if(this.id == 6)
        canvas.setAttribute("alt", "개인(?�용)?�보?? ???? 고객권리 ?�내�?");

        if(this.id == 7)
        canvas.setAttribute("alt", "????(??)?? �?��층을 ?�한 공적�??�제?? ?�내");

        if(this.id == 8)
        canvas.setAttribute("alt", "채무조정 진행?��? ?�한 공적�??? ?�도 ?�내");

        if(this.id == 9)
        canvas.setAttribute("alt", "채무조정 진행?��? ?�한 공적�??? ?�도 ?�명 ?�인??");

        if(this.id == 10)
        canvas.setAttribute("alt", "개인(?�용)?�보 ?�집/?�용/?�공 ?�약 ?�의??[?�품?�비?�안??]");
...
//    alert(viewer.style.width);
//    alert(frames['iframepdf'].document.getElementById('viewer').width);
//    alert(frames['iframepdf'].document.getElementById('viewer').style.width);
//    alert($('#iframepdf').contents().find('#viewer').width())
      return new TextLayerBuilder({
        textLayerDiv: textLayerDiv,
        pageIndex: pageIndex,
        viewport: viewport,
...
var HOSTED_VIEWER_ORIGINS = ['null',
  'http://mozilla.github.io', 'https://mozilla.github.io', 'http://127.0.0.1', 'http://localhost'];
function validateFileURL(file) {
  try {
    ...
    var inputFileUrl = new URL(file, window.location.href);
